...
import Row from 'react-bootstrap/Row';
 ...
 class pricing extends React.Component {
  ...
  state = { price: 10000, term: 12 };
  
  handleSliderChangeForTerm = (slideValue) => {
	  this.state.term = Math.round(slideValue);
	  
	  this.forceUpdate();
  };
  
  handleSliderChangeForLoan = (slideValue) => {
	  this.state.price = slideValue;
  ...
  render() {
    return (
      <div>
	   ...
	   <Slider formatValue={this.state.term+" months"} currentValue={this.state.term} changeRange={this.term.changeRange} to={this.term.to} from={this.term.from} increment={this.term.increment} onSelectValue={this.handleSliderChangeForTerm} />
       <Row className="pt-3">
